...
### Selective Re-Provisioning with Tags

Re-run only specific roles without a full rebuild:

```bash
# Just update the dcr toolkit code
ansible-playbook -i "VM_IP," playbook.yml -u deepcyber --become --tags dcr

# Just update branding
ansible-playbook -i "VM_IP," playbook.yml -u deepcyber --become --tags branding

# Just rebuild docs
ansible-playbook -i "VM_IP," playbook.yml -u deepcyber --become --tags docs
```
